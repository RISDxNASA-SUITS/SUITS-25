...
with open("test.wav", "rb") as f:
    ...
    response = requests.post("http://localhost:7070/audio", files=files)
...
poi_response = requests.post("http://localhost:7070/poi", json=poi_payload)
print("POI add response:", poi_response.text)

# 3. Retrieve POIs and extract the audioId from the first POI
get_pois = requests.get("http://localhost:7070/poi")
...
audio_response = requests.get(f"http://localhost:7070/audio/{retrieved_audio_id}")
